...
def se_block(input_feature, ratio=16, name=""):
    channel = input_feature.shape[-1]  # 获取通道数，使用 input_feature.shape
...
def eca_block(input_feature, b=1, gamma=2, name=""):
    channel = tf.cast(input_feature.shape[-1], tf.float32)  # 将通道数转换为浮点数类型

    kernel_size = int(abs((tf.math.log(channel) + b) / gamma))
...
def channel_attention(input_feature, ratio=8, name=""):
    channel = input_feature.shape[-1]  # 修改为 shape
...
def ca_block(input_feature, ratio=8, name=""):
    channel = input_feature.shape[-1]  # 修改为 shape
    h = input_feature.shape[1]  # 修改为 shape
    w = input_feature.shape[2]  # 修改为 shape
