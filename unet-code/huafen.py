    ...
    for k in range(len(save_dir)):#地址长度，目前k是两个数，0和1
        if os.path.isdir(save_dir[k]) and os.path.isdir(save_dir1[k]) and os.path.isdir()  :#判断路径是否存在
            for name in sample[k]:#sample[0]为train_sample中的数据，整句是train_sample中的数据循序进行遍历
                name1 = name.split(".")[0] + '.tif'#split()的用处是拆分字符串，1927.jpg  以 .  开始拆分成‘1927’，‘jpg’  [0]是指只用1927这个字符后面加.txt
...
if __name__ == '__main__':
    ...
    # 保存路径
    save_train_dir = 'J:/df/3/train/'#训练集图片地址
    save_test_dir = 'J:/df/3/test/'#测试集图片地址
    save_val_dir = 'J:/df/3/val/'  # 验证集图片地址
    save_train_dir1 = 'J:/df/3/train_label/'#训练集标签
    save_test_dir1 = 'J:/df/3/test_label/'#测试集标签
    save_val_dir1 = 'J:/df/3/val_label/'  # 验证集标签
